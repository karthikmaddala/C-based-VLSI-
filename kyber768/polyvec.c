...
void polyvec_compress(uint8_t r[KYBER_POLYVECCOMPRESSEDBYTES], polyvec *a)
{
  ...
  for(i=0;i<KYBER_K;i++) {
    for(j=0;j<KYBER_N/4;j++) {
      pqcrystals_kyber768_ref_polyvec_compress_label6:for(k=0;k<4;k++)
...
#elif (KYBER_POLYVECCOMPRESSEDBYTES == (KYBER_K * 320))
  uint16_t t[4];
  pqcrystals_kyber768_ref_polyvec_decompress_label3:for(i=0;i<KYBER_K;i++) {
    for(j=0;j<KYBER_N/4;j++) {
      t[0] = (a[0] >> 0) | ((uint16_t)a[1] << 8);
...
{
  unsigned int i;
  pqcrystals_kyber768_ref_polyvec_ntt_label1:for(i=0;i<KYBER_K;i++)
    poly_ntt(&r->vec[i]);
}
...
{
  unsigned int i;
  pqcrystals_kyber768_ref_polyvec_invntt_tomont_label2:for(i=0;i<KYBER_K;i++)
